...
impl Simulator for AppState {
    ...
    fn update(&mut self, to_worker: &Sender<WorkerCommand>) {
        // send new timesteps to compute
        let steps = self.frame.get_time_steps_discarded();
        if steps > 0 {
            to_worker
                .send(WorkerCommand::AddTimeStepsToCompute(steps))
                .unwrap();
        }
        ...
        for message in self.messages.drain(..) {
            match &message {
                ...
                UserInput::RequestScreenshot => {
                    self.screenshot
                        .queue_screenshot("./screenshots".to_string());
                }
                UserInput::PlayForward => {
                    self.frame.reset_steps();
                    if self.instances.finished_loop(true) {
                        self.instances
                            .allow_looping_once(self.ui.controls.is_playing_looped());
                    }
                    debug!("Play forward!");
                ...
                UserInput::RequestReadback(req) => to_worker
                    .send(WorkerCommand::SaveScreenshot(req.clone()))
                    .unwrap(),
        ...
        // Update camera
        self.camera
            .update(&self.gpu, self.frame.time_since_last_update());
