...
impl Cut {
    ...
    pub fn view(&self) -> Column<'_, UserInput> {
        let x_condition = if self.x_inverse { "<=" } else { ">=" };
        let cut_x: row::Row<'_, UserInput> = row![
            Toggler::new(self.x_active)
        ...
        let y_condition = if self.y_inverse { "<=" } else { ">=" };
        let cut_y: row::Row<'_, UserInput> = row![
            Toggler::new(self.y_active)
                .label("Show half-space for:")
        ...
        let z_condition = if self.z_inverse { "<=" } else { ">=" };
        let cut_z: row::Row<'_, UserInput> = row![
            Toggler::new(self.z_active)
                .label("Show half-space for:")
                .on_toggle(|_| UserInput::ToggleCutZ),
            text(format!(" z {z_condition} ")),
            text(self.z_bound),
            text(" "),
            button("I")
                .on_press(UserInput::FlipCutZ)
                .width(28)
                .height(28),
            button("+")
                .on_press(UserInput::CutZBoundChanged(1.))
                .width(28)
                .height(28),
            button("-")
                .on_press(UserInput::CutZBoundChanged(-1.))
