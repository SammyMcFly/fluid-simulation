//! Integration tests for `ExplicitEuler`, exercising only its public API.
//!
//! Contrasts directly with `EulerCromer`: here the position update must use
//! the *old* velocity (from the start of the step), not the newly updated
//! one — that's the defining difference between plain explicit Euler and
...
use simulation_lib::sph::integration_schemes::{ExplicitEuler, IntegrationScheme};
...
// ─── Basic explicit-Euler formula ──────────────────────────────────────────

#[test]
fn single_particle_matches_explicit_euler_update() {
...
// ─── The defining "fully explicit" property ───────────────────────────────

#[test]
fn position_update_uses_the_old_velocity_not_the_new_one() {
    // This is what distinguishes plain explicit Euler from Euler-Cromer
    // (semi-implicit Euler): position must be advanced using the velocity
    // from the *start* of the step, not the velocity already updated with
    // the current acceleration. With acc != 0 the two schemes diverge, so
    // this pins down which one is actually implemented here.
    let mut fluid = fluid_with_at_least(1);
    ...
    let dt = 1.0;
    let explicit_euler_pos = pos0 + dt * vel0; // = (1, 0, 0), expected
    let semi_implicit_pos = pos0 + dt * (vel0 + dt * acc0); // = (11, 0, 0), NOT expected

    let mut scheme = ExplicitEuler;
    scheme.integrate(&mut fluid, dt);

    assert_point_approx(fluid.position[0], explicit_euler_pos, 1e-9);
    assert!(
        (fluid.position[0] - semi_implicit_pos).norm() > 1e-6,
        "result matches semi-implicit Euler, not plain explicit Euler: {:?}",
        fluid.position[0]
    );
}
...
fn multiple_particles_are_updated_independently() {
    ...
    assert_point_approx(fluid.position[0], pos_a + dt * vel_a, 1e-9);
    assert_vector_approx(fluid.velocity[0], vel_a + dt * acc_a, 1e-9);
    assert_point_approx(fluid.position[1], pos_b + dt * vel_b, 1e-9);
    assert_vector_approx(fluid.velocity[1], vel_b + dt * acc_b, 1e-9);
}
...
fn repeated_steps_under_constant_acceleration_match_the_manual_recurrence() {
    ...
    for _ in 0..steps {
        expected_pos += dt * expected_vel; // uses velocity from BEFORE this step's update
        expected_vel += dt * g;
    }
...
#[test]
fn explicit_euler_implements_integration_scheme_default_and_clone() {
    assert_impls_integration_scheme::<ExplicitEuler>();
    let scheme = ExplicitEuler;
